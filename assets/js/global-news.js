...
const GLOBAL_API_KEY = '6efbc366-a3f6-4b0f-8161-7904491fcfbe';
...
function initGlobalSearch() {
    ...
    if (searchBtn) {
        searchBtn.addEventListener('click', function() {
            ...
            if (query) {
                window.location.href = getPathPrefix() + 'search.html?q=' + encodeURIComponent(query);
            }
        });
    }
...
async function fetchGlobalSearchSuggestions(query, autocompleteEl) {
    try {
        const url = 'https://content.guardianapis.com/search?q=' + encodeURIComponent(query) + '&api-key=' + GLOBAL_API_KEY + '&show-fields=thumbnail&page-size=8';
        ...
        articles.forEach(article => {
            ...
            html += `
                <a href="${getPathPrefix()}detail.html?id=${encodeURIComponent(article.id)}" class="autocomplete-item" onclick="document.getElementById('search-autocomplete').classList.remove('active')">
        ...
        html += `
            <div class="autocomplete-section" style="border-top: 1px solid #eee;">
                <a href="${getPathPrefix()}search.html?q=${encodeURIComponent(query)}" class="autocomplete-item" style="color: #D13E4D;">
...
function globalShareArticle(title, url) {
    const detailUrl = url ? (url.includes('detail.html') ? url : getPathPrefix() + 'detail.html?id=' + encodeURIComponent(url)) : window.location.href;
...
function globalToggleBookmark(articleId, title, thumbnail, category) {
    // TIDAK PERLU LOGIN - langsung save ke localStorage
    ...
    if (existingIndex !== -1) {
        ...
        globalShowToast('Removed from saved.');
        syncGlobalBookmarkUI(articleId, false);
    } else {
        bookmarks.unshift({
            id: articleId,
            title: title,
        ...
        globalShowToast('Article saved!');
        syncGlobalBookmarkUI(articleId, true);
    }
...
function syncAllBookmarksOnLoad() {
    const bookmarks = JSON.parse(localStorage.getItem('bookmarkedArticles')) || [];
    
    // 1. Sync category / search / global cards
    ...
    // 2. Sync dashboard/detail bookmark buttons
