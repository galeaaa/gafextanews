...
function setActiveNav() {
    ...
    navLinks.forEach(link => {
        ...
        if (linkPath && linkPath.includes('index.html') && (currentPath.endsWith('index.html') || currentPath === '/' || currentPath === '')) {
...
function renderAutocomplete(query) {
    ...
    if (matchedCategories.length > 0) {
        ...
        matchedCategories.forEach(cat => {
            html += `<a href="pages/category.html?cat=${cat.id}" class="autocomplete-item"><div class="autocomplete-item-icon"><i class="fas ${cat.icon}"></i></div><div><div class="autocomplete-item-text">${highlightMatch(cat.name, query)}</div></div></a>`;
        });
        html += '</div>';
    }
    ...
    html += `<div class="autocomplete-section" style="background: #f8f9fa; border-top: 2px solid #eee;"><a href="pages/search.html?q=${encodeURIComponent(query)}" class="autocomplete-item" style="justify-content: center; color: #ed5858; font-weight: 700;"><i class="fas fa-search"></i> Search all results for "${query}"</a></div>`;
...
function setupAutocomplete() {
    ...
    searchInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            ...
            if (query) window.location.href = `pages/search.html?q=${encodeURIComponent(query)}`;
        }
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.pathname.includes('search.html')) performSearch();
