...
const BASE_URL = `/api/news?page-size=15`;
...
function protectPage() {
    ...
    if (!isUserLoggedIn() && currentPath.includes('/profile')) {
        ...
        comments.forEach(comment => {
            ...
            commentEl.innerHTML = `
                ...
                <div class="comment-content">
                    ...
                    <div class="comment-meta"><span>${comment.time}</span><span class="comment-reply-btn">Reply</span></div>
                </div>`;
...
async function fetchNews(query = '') {
    try {
        let url = BASE_URL;
        if (query) {
            url = `/api/news?q=${encodeURIComponent(query)}&page-size=15`;
        }
        ...
        if (data.response && data.response.status === "ok") {
            ...
            if (articleId && window.location.pathname.includes('/detail')) {
...
async function fetchArticleDetail(id) {
    try {
        const detailUrl = `/api/news/detail?id=${encodeURIComponent(id)}`;
        ...
        if (article) {
            ...
            if (contentEl) {
                ...
                contentEl.innerHTML = rawText.split('\n').map(p => p.trim()).filter(p => p.length > 0).map(p => `<p>${p}</p>`).join('');
            }
            saveToReadingHistory(id, { id: id, webTitle: article.webTitle, fields: { thumbnail: article.fields?.thumbnail }, sectionName: article.sectionName });
        }
    } catch (error) {
...
function displayNews(articles) {
    ...
    if (headlineCont) {
        ...
        const formattedDate = headlineDate.toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' });
        headlineCont.innerHTML = `
            <div class="headline-box">
                ...
                <div class="headline-content">
                    ...
                    <h2 onclick="markArticleOpened('${headline.id}'); location.href='${generateDetailURL(headline)}'">${headline.webTitle}</h2>
        ...
        articles.slice(1, 5).forEach(article => {
            ...
            latestCont.innerHTML += `
                <div class="article-item">
                    ${hasValidImage 
                        ? `<img src="${thumbnail}" alt="Thumbnail" onclick="location.href='${generateDetailURL(article)}'" onerror="this.style.display='none'; this.parentElement.querySelector('.article-info').style.marginTop='0';">` 
                    ...
                    <div class="article-info">
                        ...
                        <h4 onclick="location.href='${generateDetailURL(article)}'">${article.webTitle}</h4>
    ...
    if (sidebarCont) {
        ...
        articles.slice(5, 11).forEach((article, index) => {
            ...
            const formattedDate = articleDate.toLocaleDateString('en-US', { day: 'numeric', month: 'short' });
            sidebarCont.innerHTML += `
                <a href="${generateDetailURL(article)}" class="popular-item">
                    ...
                    <div class="popular-content">
                        ...
                        <div class="popular-date"><i class="far fa-calendar"></i> ${formattedDate}</div>
                    </div>
                </a>`;
        });
    }
...
function populateTicker(articles) {
    ...
    const buildItems = () => tickerArticles.map(a => `<span class="ticker-item">${a.webTitle}</span>`).join('');
...
function setActiveNav() {
    ...
    navLinks.forEach(link => {
        ...
        if (linkPath && (linkPath === '/' || linkPath.includes('index.html')) && (currentPath === '/' || currentPath.endsWith('index.html') || currentPath === '')) {
            link.classList.add('active');
            return;
        }
        if (linkPath && (linkPath.includes('/category') || linkPath.includes('category.html'))) {
...
function toggleBookmark(event, articleId, title, thumbnail, category) {
    ...
    if (typeof globalToggleBookmark === 'function') {
        globalToggleBookmark(articleId, title, thumbnail, category);
    }
}
...
function shareArticle(event, url, title) {
    ...
    if (typeof globalShareArticle === 'function') {
...
async function performSearch() {
    ...
    if (query) {
        ...
        try {
            const url = `/api/news?q=${encodeURIComponent(query)}&page-size=20`;
            const response = await fetch(url);
            const data = await response.json();
            if (searchResults) {
                if (data.response.results.length === 0) {
                    ...
                    data.response.results.forEach(article => {
                        ...
                        const hasValidImage = thumbnail && thumbnail.startsWith('http');
                        searchResults.innerHTML += `
                            <div class="search-result-item" onclick="location.href='/detail?id=${article.id}'" style="cursor: pointer;">
                                ${hasValidImage 
...
function renderAutocomplete(query) {
    ...
    let html = '';
    const matchedCategories = searchCategories.filter(cat => cat.name.toLowerCase().includes(lowerQuery));
    if (matchedCategories.length > 0) {
        html += `<div class="autocomplete-section"><div class="autocomplete-section-title"><i class="fas fa-folder"></i> Category</div>`;
        matchedCategories.forEach(cat => {
            html += `<a href="/category?cat=${cat.id}" class="autocomplete-item"><div class="autocomplete-item-icon"><i class="fas ${cat.icon}"></i></div><div><div class="autocomplete-item-text">${highlightMatch(cat.name, query)}</div></div></a>`;
        });
        html += '</div>';
    }
    const matchedArticles = currentArticlesForAutocomplete.filter(article => article.webTitle.toLowerCase().includes(lowerQuery)).slice(0, 5);
    if (matchedArticles.length > 0) {
        html += `<div class="autocomplete-section"><div class="autocomplete-section-title"><i class="fas fa-newspaper"></i> Related Articles</div>`;
        matchedArticles.forEach(article => {
            const thumbnail = article.fields?.thumbnail;
            const hasImage = thumbnail && thumbnail.startsWith('http');
            html += `<a href="${generateDetailURL(article)}" class="autocomplete-item"><div class="autocomplete-item-icon" style="${hasImage ? `background-image: url('${thumbnail}'); background-size: cover;` : ''}">${!hasImage ? '<i class="fas fa-newspaper"></i>' : ''}</div><div><div class="autocomplete-item-text">${highlightMatch(article.webTitle, query)}</div><div class="autocomplete-item-meta">${article.sectionName || 'News'} • ${new Date(article.webPublicationDate).toLocaleDateString('en-US', { day: 'numeric', month: 'short' })}</div></div></a>`;
        });
        html += '</div>';
    }
    if (matchedCategories.length === 0 && matchedArticles.length === 0) {
        html += `<div class="autocomplete-no-results"><i class="fas fa-search"></i><div>No results found for "<strong>${query}</strong>"</div><div style="font-size: 11px; margin-top: 4px;">Press Enter to search</div></div>`;
    }
    html += `<div class="autocomplete-section" style="background: #f8f9fa; border-top: 2px solid #eee;"><a href="/search?q=${encodeURIComponent(query)}" class="autocomplete-item" style="justify-content: center; color: #ed5858; font-weight: 700;"><i class="fas fa-search"></i> Search all results for "${query}"</a></div>`;
...
function setupAutocomplete() {
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        if (query.length >= 2) {
            searchTimeout = setTimeout(() => renderAutocomplete(query), 300);
        } else {
            const dropdown = document.getElementById('search-autocomplete');
    ...
    searchInput.addEventListener('focus', () => {
        const query = searchInput.value.trim();
        if (query.length >= 2) renderAutocomplete(query);
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* Autocomplete handled by global-news.js */
